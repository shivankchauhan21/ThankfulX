...
const CREDIT_MULTIPLIERS: CreditMultipliers = {
  length: {
    short: 1,
    medium: 1.5,
    long: 2
...
export function calculateCreditCost(
  ...
  options: MessageOptions & { creativity?: number; hasProducts?: boolean }
): number {
...
export function getCreditCalculationExplanation(
  customerCount: number,
  options: MessageOptions & { creativity?: number; hasProducts?: boolean }
): string {
  const { length, style, language, creativity = 0.7, hasProducts = false } = options;
